...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  location            = each.value.location
  size                = "Standard_B1s"
  admin_username      = data.azurerm_key_vault_secret.keyvaultsecret1[each.key].value
  admin_password      = data.azurerm_key_vault_secret.keyvaultsecret2[each.key].value
